...
def flip_segms_vertical(segms, height, width):
    """Top/Bottom flip each mask in a list of masks."""

    def _flip_poly_vertical(poly, height):
        flipped_poly = np.array(poly)
        flipped_poly[1::2] = height - np.array(poly[1::2]) - 1
        return flipped_poly.tolist()
    ...
    for segm in segms:
        if type(segm) == list:
            # Polygon format
            flipped_segms.append([_flip_poly_vertical(poly, height) for poly in segm])
        else:
            # RLE format
            assert type(segm) == dict
            print("Sorry, no support for vertical flips of RLE masks :(")
    return flipped_segms
    pass
